...
while [[ $# -gt 0 ]]; do
  case "$1" in
  --skip-loading) SKIP_LOADING=true; shift ;;
...
run_test() {
  ...
  if [[ ${VERBOSE} == true ]]; then echo "=== ${test_name} [${plugin_path}] ==="; fi
...
if [[ ${#PLUGINS[@]} -eq 0 && -f plugin.json ]]; then PLUGINS+=("."); fi
...
if [[ ${FAILED_TESTS} -gt 0 ]]; then echo "Failed: ${FAILED_TESTS}"; exit 1; fi
